...
Blockly.Toolbox = function(workspace) {
  ...
  this.config_ = {
    ...
    cssItemLabel: 'blocklyTreeLabel',//cssItemLabel: 'blocklyTreeLabel',
  ...
  Blockly.bindEventWithChecks_(this.HtmlDiv, 'mousedown', this,
      function(e) {
        if (Blockly.utils.isRightButton(e) || e.target == this.HtmlDiv) {
          // Close flyout.
          Blockly.hideChaff(false);  //* Close tooltips, context menus, dropdown selections, etc.
        } else {
          // Just close popups.
...
Blockly.Toolbox.prototype.addColour_ = function(opt_tree) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    if (element) {
      if (this.hasColours_) {
        var border = '1px solid ' +  'rgb(197,198,202)';//'8px solid ' + (child.hexColour || '#ddd');
      } else {
        var border = 'none';
      }
      if (this.workspace_.RTL) {
        element.style.borderRight = 'none';//border 不添加boder
        element.style.borderBottom= 'none';
      } else {
        element.style.borderLeft = 'none';//border 不添加boder
        element.style.borderBottom= 'none';//border
...
Blockly.Toolbox.TreeControl.prototype.setSelectedItem = function(node) {
  ...
  if (node) {
    ...
    if(node.id_ == ":1")
    {
      ...
      //   else {
      //
      //     node.getRowElement().style.cursor = 'wait';
      //     // var CheckCode = false;
      //     // var compileMessage = "";
      //     // if(Code.checkWorkspaceToCSharp())
      //     // {
      //     //   if(bound.csharpCompileResult(Code.generateCSharp())) {
      //     //     CheckCode = true;
      //     //   }
      //     //   else
      //     //   {
      //     //     compileMessage = bound.csharpCompileExceptionMessage(Code.generateCSharp());
      //     //   }
      //     // }
      ...
      /**
       //var filexml = bound.loadXML("main");
       //var currentxml = Blockly.Xml.workspaceToDom(Code.workspace);
       //if(filexml != currentxml)
       {
       //  node.getRowElement().style.cursor = 'default';
       //  window.alert("当前空间与保存文件不一致，是否保存当前!");
       ...
       //var CheckCode = Code.saveCSharpFileToCSharp("main");
       //if(CheckCode) {
      //  Code.saveXMLFileToCSharp("main");
      ...
      //}
       ...
       //{
      //  node.getRowElement().style.cursor = 'default';
      //  window.alert("编译出错,保存失败!\r\n 请注意红色带感叹号❗的模块,单击可以显示出错问题!");
      //}
       */
    }
  }
