...
/**
 * 是否为 APP/WebView 环境（由宿主在加载后设置，如 code.js 中 Blockly.IS_APP = typeof bound !== 'undefined'）。
 * 为 true 时块内输入框、注释等使用 Blockly.prompt 弹窗输入，避免 WebView 内 input 无法聚焦/弹不出键盘。
 * @type {boolean}
 */
Blockly.IS_APP = false;
...
Blockly.copy_ = function(block) {
  ...
  // Copy only the selected block and internal blocks.仅复制选定的块和内部块。
...
Blockly.copyd_ = function(block) {
  var xmlBlock = Blockly.Xml.blockToDom(block);
  // Copy only the selected block and internal blocks.仅复制选定的块和内部块。
  Blockly.Xml.deleteNext(xmlBlock);
  // Encode start position in XML.
  var xy = block.getRelativeToSurfaceXY();
  //alert( block.RTL ? -xy.x : xy.x);
  xmlBlock.setAttribute('x', block.RTL ? -xy.x : xy.x);
  xmlBlock.setAttribute('y', xy.y);
  Blockly.copyXml_ = xmlBlock;
  Blockly.copySpace_ = block.workspace;
};

Blockly.paste_ = function(block) {

  var xy = block.getRelativeToSurfaceXY();
  Blockly.copyXml_.setAttribute('x', block.RTL ? -xy.x : xy.x);
  Blockly.copyXml_.setAttribute('y', xy.y);
  block.workspace.paste(Blockly.copyXml_ );
...
/**
 * 从JSON中定义块的帮助函数。得到的函数有
  *在代码中调用jsonInit时，jsonDef的正确值。
