...
/**
 * Height of PLC-style inline comment row below block (px).
 * @const
 */
Blockly.BlockSvg.COMMENT_LABEL_HEIGHT = 22;
/**
...
Blockly.BlockSvg.prototype.renderDraw_ = function(iconWidth, inputRows) {
  ...
  if (this.comment) {
    // 从 XML 加载时 Comment 在块未渲染时创建，commentLabelGroup_ 未挂到 DOM，此处补挂
    if (this.comment.commentLabelGroup_ && !this.comment.commentLabelGroup_.parentNode) {
      var root = this.getSvgRoot();
      if (root) root.appendChild(this.comment.commentLabelGroup_);
    }
    var commentH = this.comment.getCommentLabelHeight ?
        this.comment.getCommentLabelHeight() : Blockly.BlockSvg.COMMENT_LABEL_HEIGHT;
    this.comment.positionInlineLabel_(this.height - commentH, this.width);
  }
};
...
Blockly.BlockSvg.prototype.renderDrawBottom_ =
  ...
  // PLC-style: reserve space for inline comment below block（自适应高度，无翻页）.
  var commentHeight = 0;
  if (this.comment) {
    commentHeight = this.comment.getCommentLabelHeight ?
        this.comment.getCommentLabelHeight() : Blockly.BlockSvg.COMMENT_LABEL_HEIGHT;
  ...
  if (this.nextConnection) {
    ...
    // Create next block connection. PLC-style: put connection below comment row.
    var connectionX;
    if (this.RTL) {
    ...
    var nextConnY = cursorY + 1;
    if (this.comment) {
      nextConnY += commentHeight;
    }
    this.nextConnection.setOffsetInBlock(connectionX, nextConnY);
