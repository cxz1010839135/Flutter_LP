...
/**
 * 返回block所有连接block对象，包含输出、输入、前一个、后一个block
...
/**
 * 如果是HSV 的H输入，转化HSV值到RGB ，如果RGB表示直接用RBG
 * 添加颜色输入判断 %BKY_BLOCK_CONTROL_RGB
 * goog.isString(colour) && colour.match(/%{BKY_([a-zA-Z][a-zA-Z0-9_]*)}/g)
...
Blockly.Block.prototype.setColour = function(colour) {
  ...
  if (!isNaN(hue)) {
    ...
    this.colour_ = colour;
  } else if(goog.isString(colour) && colour.match(/%{BKY_([a-zA-Z][a-zA-Z0-9_]*)}/g)){
    colour=Blockly.utils.replaceMessageReferences(colour);
 ...
 * 返回此块左上角相对于绘图表面原点 (0,0) 的坐标（以工作区单位为单位）。
