 ...
 */
Blockly.Variables.NAME_TYPE = Blockly.VARIABLE_CATEGORY_NAME;//Blockly.VARIABLE_CATEGORY_NAME = 'VARIABLE';
...
Blockly.Variables.flyoutCategory = function(workspace) {
  var xmlList = [];
  /* 取消变量Flyout中的创建变量按钮，在dropdown中重新创建新变量选项
...
Blockly.Variables.flyoutCategoryBlocks = function(workspace) {
  ...
  if (variableModelList.length > 0) {
    ...
    //暂时不支持变量类型转换 string bool double int
    /*
  ...
  else if(variableModelList.length == 0) //添加默认变量
  {
    //workspace.createVariable(Blockly.Msg.VARIABLES_DEFAULT_NAME);
  }
  return xmlList;
};
...
Blockly.Variables.generateUniqueName = function(workspace) {
  ...
  if (variableList.length) {
    var nameSuffix = 1;
    var letters = 'V';//'ijkmnopqrstuvwxyzabcdefgh';  // No 'l'.
    var letterIndex = 0;
    var potName = letters.charAt(letterIndex)+ nameSuffix;//letters.charAt(letterIndex);
    while (!newName) {
      var inUse = false;
      for (var i = 0; i < variableList.length; i++) {
          if (variableList[i].name.toUpperCase() == potName) {//if (variableList[i].name.toLowerCase() == potName) {
