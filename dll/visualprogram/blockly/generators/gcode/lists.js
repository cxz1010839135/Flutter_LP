...
goog.provide('Blockly.GCode.lists');

goog.require('Blockly.GCode');

Blockly.GCode['lists_create_empty'] = function(block) {
  return ['null', Blockly.GCode.ORDER_ATOMIC];
};

Blockly.GCode['lists_create_with'] = function(block) {
  // Create a list with any number of elements of any type.
  var code = new Array(block.itemCount_);
  for (var n = 0; n < block.itemCount_; n++) {
    code[n] = Blockly.GCode.valueToCode(block, 'ADD' + n,
...
Blockly.GCode['lists_repeat'] = function(block) {
  // Create a list with one element repeated.
  if (!Blockly.GCode.definitions_['lists_repeat']) {
    ...
    Blockly.GCode.lists_repeat.repeat = functionName;
    var func = [];
    ...
    func.push('});');
    Blockly.GCode.definitions_['lists_repeat'] = func.join('\n');
  }
  var argument0 = Blockly.GCode.valueToCode(block, 'ITEM',
      Blockly.GCode.ORDER_NONE) || 'null';//Blockly.GCode.ORDER_COMMA
  var argument1 = Blockly.GCode.valueToCode(block, 'NUM',
      Blockly.GCode.ORDER_NONE) || '0';//Blockly.GCode.ORDER_COMMA
  var code = Blockly.GCode.lists_repeat.repeat +
      '(' + argument0 + ', ' + argument1 + ')';
  return [code, Blockly.GCode.ORDER_UNARY_POSTFIX];//Blockly.GCode.ORDER_FUNCTION_CALL
};

Blockly.GCode['lists_length'] = function(block) {
  // List length.
  var argument0 = Blockly.GCode.valueToCode(block, 'VALUE', Blockly.GCode.ORDER_UNARY_POSTFIX) || 'null';//Blockly.GCode.ORDER_FUNCTION_CALL
  return [argument0 + '.Count', Blockly.GCode.ORDER_UNARY_POSTFIX];//Blockly.GCode.ORDER_MEMBER
};

Blockly.GCode['lists_isEmpty'] = function(block) {
  // Is the list empty?
  var argument0 = Blockly.GCode.valueToCode(block, 'VALUE',
      Blockly.GCode.ORDER_UNARY_POSTFIX) || 'null';//Blockly.GCode.ORDER_MEMBER
  return [argument0 + '.Count == 0', Blockly.GCode.ORDER_UNARY_PREFIX];//Blockly.GCode.ORDER_LOGICAL_NOT
};

Blockly.GCode['lists_indexOf'] = function(block) {
  // Find an item in the list.
  var operator = block.getFieldValue('END') == 'FIRST' ?
      'IndexOf' : 'LastIndexOf';
  var argument0 = Blockly.GCode.valueToCode(block, 'FIND',
      Blockly.GCode.ORDER_NONE) || 'null';
  var argument1 = Blockly.GCode.valueToCode(block, 'VALUE',
      Blockly.GCode.ORDER_UNARY_POSTFIX) || 'null';//Blockly.GCode.ORDER_MEMBER
  var code = argument1 + '.' + operator + '(' + argument0 + ') + 1';
  return [code, Blockly.GCode.ORDER_UNARY_POSTFIX];//Blockly.GCode.ORDER_MEMBER
};

Blockly.GCode['lists_getIndex'] = function(block) {
  var mode = block.getFieldValue('MODE') || 'GET';
  var where = block.getFieldValue('WHERE') || 'FROM_START';
  var at = Blockly.GCode.valueToCode(block, 'AT',
      Blockly.GCode.ORDER_UNARY_PREFIX) || '1';//Blockly.GCode.ORDER_UNARY_NEGATION
  var list = Blockly.GCode.valueToCode(block, 'VALUE',
      Blockly.GCode.ORDER_UNARY_POSTFIX) || 'null';//Blockly.GCode.ORDER_MEMBER
  ...
  if (mode == 'GET_REMOVE') {
    ...
    if (!Blockly.GCode.definitions_['lists_get_remove_at']) {
    var functionName = Blockly.GCode.variableDB_.getDistinctName(
        'lists_get_remove_at', Blockly.Generator.NAME_TYPE);
    Blockly.GCode.lists_getIndex.lists_get_remove_at = functionName;
    var func = [];
    ...
    func.push('});');
    Blockly.GCode.definitions_['lists_get_remove_at'] =
        func.join('\n');
    }
    code = Blockly.GCode.lists_getIndex.lists_get_remove_at +
        '(' + list + ', ' + at + ')';
    return [code, Blockly.GCode.ORDER_UNARY_POSTFIX];//Blockly.GCode.ORDER_FUNCTION_CALL
  }
  ...
  if (where == 'FIRST') {
    ...
    if (!Blockly.GCode.definitions_['lists_get_random_item']) {
      var functionName = Blockly.GCode.variableDB_.getDistinctName(
          'lists_get_random_item', Blockly.Generator.NAME_TYPE);
      Blockly.GCode.lists_getIndex.random = functionName;
      var func = [];
      ...
      func.push('  }');
      func.push('});');
      Blockly.GCode.definitions_['lists_get_random_item'] =
          func.join('\n');
    }
    code = Blockly.GCode.lists_getIndex.random +
        '(' + list + ', ' + (mode != 'GET') + ')';
    if (mode == 'GET') {
      return [code, Blockly.GCode.ORDER_UNARY_POSTFIX];//Blockly.GCode.ORDER_FUNCTION_CALL
    } else if (mode == 'REMOVE') {
...
Blockly.GCode['lists_setIndex'] = function(block) {
  // Set element at index.
  var list = Blockly.GCode.valueToCode(block, 'LIST',
      Blockly.GCode.ORDER_UNARY_POSTFIX) || 'null';//Blockly.GCode.ORDER_MEMBER
  var mode = block.getFieldValue('MODE') || 'GET';
  var where = block.getFieldValue('WHERE') || 'FROM_START';
  var at = Blockly.GCode.valueToCode(block, 'AT',
      Blockly.GCode.ORDER_NONE) || '1';
  var value = Blockly.GCode.valueToCode(block, 'TO',
  ...
  if (where == 'FIRST') {
    ...
    var xVar = Blockly.GCode.variableDB_.getDistinctName(
...
Blockly.GCode['lists_getSublist'] = function(block) {
  // Get sublist.
  var list = Blockly.GCode.valueToCode(block, 'LIST',
      Blockly.GCode.ORDER_UNARY_POSTFIX) || 'null';//Blockly.GCode.ORDER_MEMBER
  var where1 = block.getFieldValue('WHERE1');
  var where2 = block.getFieldValue('WHERE2');
  var at1 = Blockly.GCode.valueToCode(block, 'AT1',
      Blockly.GCode.ORDER_NONE) || '1';
  var at2 = Blockly.GCode.valueToCode(block, 'AT2',
      Blockly.GCode.ORDER_NONE) || '1';
  if (where1 == 'FIRST' && where2 == 'LAST') {
    var code = 'new List<dynamic>(' + list + ')';
  } else {
    if (!Blockly.GCode.definitions_['lists_get_sublist']) {
      var functionName = Blockly.GCode.variableDB_.getDistinctName(
          'lists_get_sublist', Blockly.Generator.NAME_TYPE);
      Blockly.GCode.lists_getSublist.func = functionName;
      var func = [];
      ...
      func.push('});');
      Blockly.GCode.definitions_['lists_get_sublist'] =
          func.join('\n');
    }
    var code = Blockly.GCode.lists_getSublist.func + '(' + list + ', "' +
