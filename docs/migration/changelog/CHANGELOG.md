...
**类型**：连接页网络识别 + 操控单选 + 安卓视口/点动

### 新增

- **连接页标签**：已连 Wi‑Fi 时显示「当前连接WIFI：SSID」；网线/未连 Wi‑Fi 仍显示「控制器IP」
- **Android 读 SSID**：首次进入连接页在已连 Wi‑Fi 时申请附近设备/定位权限，以便显示热点名称

### 优化

- **操控点动模式**：连续 / 长 / 中 / 短四格互斥，同一时间只高亮一格
- **操控左右导航**：X/Y/Z/I/O 与 关节/门型/直线 互斥单选，不再两侧同时高亮
- **点动 ±**：Windows 鼠标移出按钮后仍能松开停止连续点动
- **Android 视口**：设计稿缩放避开系统栏/手势区；去掉主题 `windowFullscreen`，减少虚拟导航键遮挡

### 构建

- 版本号 **2.1.2+1**；Windows Setup/MSI + Android APK
...
- **界面清零**：连接时清零角读取顺序对齐安卓 ConnectActivity.initAll（先 zeroangle，再按机型/型号覆盖）；Scara/440C 二轴预设恢复 0 / 4.78 / -4.78
- **协议机型映射**：连接 robot.type 映射对齐安卓（含 Axis5/6、码垛等）
- **清零页**：有预设时支持前部输入 + 右侧箭头选择；zeroangle=0 时去重避免下拉崩溃

### 优化

- **操控点动**：默认关节 + X、中距离；模式在 XYZ/关节间保持
- **点编号**：有效范围 0-999
- **运行中进编程**：增加离线编程确认提示

### 构建

- 版本号 **2.0.7+1**；Windows Setup/MSI + Android APK

---

## [2.0.6] - 2026-08-17

**类型**：操控点动默认态 + 内页顶栏 + Blockly 体验 + 说明书图文校正

### 优化

- **操控页**：默认选中关节 + X，点动模式默认中距离；切换 X/Y/Z/关节时保留连续/长/中/短选中
- **内页顶栏**：标题贴第二色块左侧，返回贴右；编程页 AI/刷新图标
- **Blockly**：编程页切图、安卓拖拽结束闪屏修复、Windows 拖拽轻量路径
- **软著说明书**：V2.0.5 界面截图图文对齐（监控/编程等错位修正）

### 构建

- 版本号 **2.0.6+1**；Windows Setup/MSI + Android APK
