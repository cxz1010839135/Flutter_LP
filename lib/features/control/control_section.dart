...
enum ControlSection {
  ...
  /// 显示关节点动面板（controlIndex 4）。
  bool get showsJointJogPanel => this == ControlSection.joint;

  /// 显示 X/Y/Z/关节 共用点动面板。
  bool get showsJogPanel => showsCartesianJogPanel || showsJointJogPanel;

  /// 显示门型 / 直线定位面板（controlIndex 5–6）。
  bool get showsMovePanel =>
      this == ControlSection.gantry || this == ControlSection.linear;

  /// 显示 IO 读写面板（controlIndex 3）。
  bool get showsIoPanel => this == ControlSection.io;

  /// 点动轴号：X=0, Y=1, Z=2（与 Android `axisIndex` 一致）。
  int? get jogAxisIndex => showsCartesianJogPanel ? controlIndex : null;

  bool get isLeftNav => leftNav.contains(this);

  bool get isRightNav => rightNav.contains(this);
